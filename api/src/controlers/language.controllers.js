...
const getLanguages= async (req, res) => {
    try{
        ...
        const result = await connection.query("SELECT id_factura,cantidad_producto,id_producto,nombre_producto,precio_producto,total,id_cliente FROM facturas");
        res.json(result);
    }catch(error){
        res.status(500);
...
const deleteLanguage= async (req, res) => {
    try{
        ...
        const connection = await getConnection();
        const result = await connection.query("DELETE FROM facturas WHERE id_factura = ?", id_producto);
        res.json(result);
    }catch(error){
        res.status(500);
        res.send(error.message);
